...
public class HomeJobAdapter extends RecyclerView.Adapter<HomeJobAdapter.ViewHolder> {
    ...
    private final List<JobListBean> mValues;
    ...
    public HomeJobAdapter(List<JobListBean> items, Context context) {
        mValues = items;
        mContext = context;
    }
...
//            mAnimationDrawable.start();
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if (mValues.size() + 1 == position) {
            ...
            final JobListBean job = mValues.get(position-1);
            int money = job.getMoney();
//            if(money.indexOf(".") > 0){
//                //正则表达
...
//            }
            ...
            String date= DateUtils.getTime(job.getStart_date(),job.getEnd_date());
...
//            }
            ...
            int count=job.getUser_count();//已报名人数
